...
BMI_all <- function(){
    ...
    env.data.lalo <- env.data.lalo[,-c(14:17)]
    colnames(env.data.lalo)[12:13] <-c("latitude","longitude")
    ...
    env.data.lalo.tool[,c(20:23)] <- tool.matrix
    colnames(env.data.lalo.tool)[20:23] <- colnames(tool.matrix)


    floodplain <- env.data.lalo.tool[,c(46:51)]
    ...
    env.data.lalo.tool.fp[,46] <- aa.1
    colnames(env.data.lalo.tool.fp)[46] <- "Flood_plain(1 natural, 2 agriculture area, 3 road, 4 parking lot, 5 trail, 6 etc)"
    env.data.lalo.tool.fp <- env.data.lalo.tool.fp[,-c(47:51)]
    colnames(env.data.lalo.tool.fp)
    env.data.conversion <- data.frame(env.data.lalo.tool.fp[,9],
                                      env.data.lalo.tool.fp[,2:8],
                                      env.data.lalo.tool.fp[,12:13],
                                      env.data.lalo.tool.fp[,10:11],
                                      env.data.lalo.tool.fp[,14:15],
                                      env.data.lalo.tool.fp[,24:25],
                                      env.data.lalo.tool.fp[,20:23],
                                      env.data.lalo.tool.fp[,26:56],
                                      env.data.lalo.tool.fp[,72:78],
                                      env.data.lalo.tool.fp[,57:61],
                                      env.data.lalo.tool.fp[,68:71],
                                      env.data.lalo.tool.fp[,79:80],
                                      env.data.lalo.tool.fp[,18:19],
                                      env.data.lalo.tool.fp[,17]
    )

    colnames(env.data.conversion)[c(1,72)] <- c("site_code","investigate_special_note")
